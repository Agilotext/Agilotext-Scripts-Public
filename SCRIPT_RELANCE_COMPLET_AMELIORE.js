...
(function() {
  ...
  async function relancerCompteRendu() {
    ...
    if (!summaryExists) {
      ...
      const proceed = confirm(
        ...
        'Si c\'est la première fois, vous devez d\'abord générer un compte-rendu via le formulaire d\'upload.\n\n' +
  ...
  function init() {
    ...
    // Normaliser la largeur du compte-rendu après chaque chargement
  ...
  function normalizeSummaryWidth() {
    ...
    tables.forEach(table => {
      ...
      // Gérer aussi l'attribut width (ex: width="85%")
      const widthAttr = table.getAttribute('width');
      if (widthAttr && /^\d+%$/.test(widthAttr)) {
        table.setAttribute('width', '100%');
      }
      
      // Gérer aussi l'attribut align (ex: align="center")
    ...
    const elements = summaryEditor.querySelectorAll('[style*="width"], [width]');
    elements.forEach(el => {
      ...
      // Gérer aussi l'attribut width
      const widthAttr = el.getAttribute('width');
      if (widthAttr && /^\d+%$/.test(widthAttr)) {
        el.setAttribute('width', '100%');
      }
      
      // Gérer aussi l'attribut align
  ...
  function setupSummaryWidthNormalizer() {
    ...
    observer.observe(summaryEditor, {
      ...
      subtree: true,
      attributes: true,
      attributeFilter: ['style', 'width', 'align']
    });
    ...
    // Normaliser aussi après rechargement de page (si tab=summary dans l'URL)
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.get('tab') === 'summary') {
      setTimeout(normalizeSummaryWidth, 500);
    }
  }
  
  // Exposer la fonction globalement pour pouvoir l'appeler manuellement si besoin
  ...
  if (!document.querySelector('#relance-summary-styles')) {
    ...
    style.textContent = `
      /* =====================================================================
         FORCER LE COMPTE-RENDU À PRENDRE TOUTE LA LARGEUR
         ===================================================================== */
      
      /* Forcer toutes les tables du compte-rendu à 100% */
      #summaryEditor table,
      #summaryEditor > table {
        width: 100% !important;
      ...
      /* S'assurer que les cellules prennent aussi toute la largeur */
      #summaryEditor td,
      #summaryEditor th {
        width: 100% !important;
        max-width: 100% !important;
      ...
      /* Éviter les marges auto qui centrent */
      #summaryEditor [style*="margin:auto"],
      #summaryEditor [style*="margin: auto"],
      #summaryEditor [style*="margin-left:auto"],
      #summaryEditor [style*="margin-right:auto"] {
        margin: 0 !important;
        margin-left: 0 !important;
        margin-right: 0 !important;
      }
      
      /* Forcer width à 100% pour les éléments avec width: 70% ou similaire */
      #summaryEditor [style*="width:70%"],
      #summaryEditor [style*="width: 70%"],
      #summaryEditor [style*="width:75%"],
      #summaryEditor [style*="width: 75%"],
      #summaryEditor [style*="width:80%"],
      #summaryEditor [style*="width: 80%"],
      #summaryEditor [style*="width:85%"],
      #summaryEditor [style*="width: 85%"] {
        width: 100% !important;
      }
      
      /* Gérer aussi les attributs width (ex: width="85%") */
      #summaryEditor table[width],
      #summaryEditor [width] {
        width: 100% !important;
      }
      
      /* Retirer l'alignement center */
      #summaryEditor table[align="center"],
      ...
      /* Éviter les padding/margin qui créent des espaces */
      #summaryEditor > *:first-child {
        margin-left: 0 !important;
        margin-right: 0 !important;
      }
      
      /* =====================================================================
         LOADER (styles existants)
         ===================================================================== */
