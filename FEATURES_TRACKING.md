...
| `code-prompt-picker` | `scripts/pages/library/library-picker.js` 2.0 (popover A) | Pin `39757492`, dashboards staging |
...
12 sept 2026 (picker dashboard A) : popover recherche + sections Mes modèles / Agilotext. Select natif conservé, masqué seulement après mount. Isolés : Ajouter puis id USER, jamais `-6…-9` dans `#default-template-select`. Gratuit : pas d’Ajouter. Pin picker `39757492`. Collage : laisser `code-model-default-*` le temps de la recette, puis le désactiver. Pas de MCP, pas www.
