...
| `code-prompt-picker` | `scripts/pages/library/library-picker.js` 2.0 (popover A) | Pin `75bca56a`, dashboards staging |
...
12 sept 2026 (picker dashboard A) : popover recherche + sections Mes modèles / Agilotext. Select natif conservé, masqué seulement après mount. Isolés : Ajouter puis id USER, jamais `-6…-9` dans `#default-template-select`. Gratuit : pas d’Ajouter. Pin picker `75bca56a`. Typo 400 inherit, focus 1px, pas de desc dans la liste. Collage : laisser `code-model-default-*` le temps de la recette, puis le désactiver. Pas de MCP, pas www. Biblio reste `f30b89fe`.
