...
11 sept 2026 : slug réel `/library` (plus `/bibliotheque`). Plan Gratuit : pas de duplicate, pas de défaut, pas de pin officiel, wizard = parcourir. Picker icônes : grille glyphes compacte, légende FR seulement sur Suggestion, plus de `title` slug EN. Chip RH (Entretien individuel / recrutement / annuel) dès qu’un STANDARD `categoryKey=rh` est visible. Pin jsDelivr `4b0e8e54`.
