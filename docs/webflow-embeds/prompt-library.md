...
|------|------|-----------------|
| `library2Live` | Appeler `/api/v1/library2/` | `true` (staging) |
| `cse89Live` | CTA achat 890 € / 89 € | `true` (staging) : payeur `hasCse` → Ajouter / copie USER, non payeur → `/offres/cse` |
...
STANDARD `promptModelId === 7` (ancien PV CSE) : **masqué** pour tout le monde (picker + catalogue). ID `5` (PV générique ancien) reste.

## Fichiers

```
scripts/pages/library/library.css
scripts/pages/library/library-api.js          (1.6.0)
...
scripts/pages/library/library-picker.js       (2.3.0)
...
<script src="https://cdn.jsdelivr.net/gh/Agilotext/Agilotext-Scripts-Public@SHA/scripts/pages/library/library-standards-meta.js?v=SHA"></script>
...
<script src="https://cdn.jsdelivr.net/gh/Agilotext/Agilotext-Scripts-Public@SHA/scripts/pages/library/library-overlay.js?v=SHA"></script>
<script src="https://cdn.jsdelivr.net/gh/Agilotext/Agilotext-Scripts-Public@SHA/scripts/pages/library/library-icon-picker.js?v=SHA"></script>
...
- `pln_cse-ic00nme` (préfixe `pln_cse-`) → `/app/business/dashboard`
- `prc_cse89y-vf20nyv` / `prc_cse89-rr10n0l` → `/app/business/dashboard`
- `pln_pack-cse` (cadeau) **ne** route **pas** vers business

Ticket Java parallèle (hors ce repo) : `member-access.businessTypes` doit contenir `cse` dès qu’un plan actif matche **`pln_cse-`**. Le front n’invente pas le pack.
