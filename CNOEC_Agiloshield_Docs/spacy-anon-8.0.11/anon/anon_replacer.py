...
class AnonReplacer:
    ...
    def build_patterns():
        ...
        return {
            ...
            # URSSAF: capture ONLY the value written after "URSSAF"
            "URSSAF_LINE": re.compile(
                r"(?im)\burssaf\b\s*(?:[:#-]\s*|n[°o]\s*|num(?:e|é)ro\s*)?"
                r"((?:\d[\s.\-]*){9,18})"
            ),
    ...
    def find_regex_spans(text: str, patterns, cell_mode: bool = False):
        ...
        # SIREN — after NIR/VAT/SIRET; skip if this match is inside an existing NIR/VAT/SIRET span
        for m in patterns["SIREN"].finditer(text):
            ...
            for existing in spans:
                if existing["placeholder"] in ("[NIR]", "[SIRET]", "[VAT]") and \
