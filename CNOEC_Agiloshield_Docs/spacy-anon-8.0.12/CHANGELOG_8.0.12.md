...
- `anon/anon_replacer.py`
  ...
  - `URSSAF_LINE` renforcé: accepte `URSSAF <région> n° ...` et limite les faux positifs montants.
  - Ajout de `RCS_SIREN_LINE` pour capter le SIREN dans les lignes `RCS <ville> <9 chiffres>`.
  - Garde anti-chevauchement: exclusion des spans SIRET/SIREN quand la zone est déjà couverte par un IBAN.
- `anon/anon_processor.py`
  - Alignement du filtre montant/SIREN post-regex avec les nouveaux mots-clés comptables.
  - Stoplist NER/name-regex durcie sur termes comptables (`rcs`, `bpm`, `cpt`, `cotisations`, etc.) pour limiter les faux `[PERSON]`.
- `anon/version.py`
...
- Réduction des fuites URSSAF avec région (`URSSAF PACA n° ...`).
- Réduction des collisions de type IBAN partiellement masqué / sous-spans SIRET-SIREN.
