...
export function validateFilePath(filePath, allowedDirs) {
    ...
    if (allowedDirs && allowedDirs.length > 0) {
        ...
        if (!isAllowed) {
            throw new Error(`Invalid file path: path must be within allowed directories`);
        }
    }
    // Verify file exists
 ...
 */
export function validateFileSize(size, maxSize = DEFAULT_MAX_SIZE) {
    ...
    if (size > maxSize) {
        ...
        throw new Error(`File too large: ${actualSizeMB}MB exceeds maximum of ${maxSizeMB}MB`);
    }
}
/**
 * Validates file extension against allowed list
 */
export function validateFileExtension(filename, allowedExtensions = DEFAULT_ALLOWED_EXTENSIONS) {
 ...
 */
export function validateMimeType(buffer, filename, allowedTypes = DEFAULT_ALLOWED_MIME_TYPES) {
    ...
    if (!detectedType || !allowedTypes.includes(detectedType)) {
        throw new Error(`Invalid file type: ${detectedType || "unknown"}. Allowed types: ${allowedTypes.join(", ")}`);
    }
}
/**
 ...
 */
export function validateFile(filePath, fileBuffer, options = {}) {
    const { maxSizeBytes = DEFAULT_MAX_SIZE, allowedExtensions = DEFAULT_ALLOWED_EXTENSIONS, allowedMimeTypes = DEFAULT_ALLOWED_MIME_TYPES, allowedDirs, } = options;
