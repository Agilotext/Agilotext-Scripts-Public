...
export function validateJobId(jobId) {
    ...
    if (jobId.length > MAX_JOB_ID_LENGTH) {
        throw new Error(`Invalid jobId: length ${jobId.length} exceeds maximum of ${MAX_JOB_ID_LENGTH}`);
    }
    ...
    if (!isUUID && !isAlphanumeric) {
        throw new Error("Invalid jobId format: must be alphanumeric, UUID, or contain only hyphens/underscores");
    }
}
/**
...
export function validateStringLength(value, maxLength, fieldName) {
    ...
    if (byteLength > maxLength) {
        ...
        throw new Error(`Invalid ${fieldName}: size ${actualLengthKB}KB exceeds maximum of ${maxLengthKB}KB`);
    }
}
/**
 * Validates email thread content
 */
export function validateEmailThread(emailThread) {
    if (!emailThread) {
 ...
 */
export function validateTranscriptText(transcriptText) {
    if (!transcriptText) {
        return; // Empty is allowed
    }
    validateStringLength(transcriptText, MAX_TRANSCRIPT_TEXT_LENGTH, "transcriptText");
...
export function validateUrl(url, fieldName = "url") {
    ...
    if (url.length > MAX_URL_LENGTH) {
        throw new Error(`Invalid ${fieldName}: length exceeds maximum of ${MAX_URL_LENGTH}`);
    }
 ...
 */
export function detectInjectionAttempts(input, fieldName) {
    ...
    for (const pattern of INJECTION_PATTERNS) {
        if (pattern.test(input)) {
            ...
            if (typeof console !== 'undefined' && console.warn) {
                console.warn(`Potential injection attempt detected in ${fieldName}: ${pattern.toString()}`);
            }
...
export function validateFilename(filename) {
    ...
    if (filename.length > MAX_FILENAME_LENGTH) {
        throw new Error(`Invalid filename: length exceeds maximum of ${MAX_FILENAME_LENGTH}`);
    }
