 ...
 */
export function translateWebhookError(error, context = {}) {
    ...
    if (isWebhook && (errorMessage.includes('webhook') ||
 ...
 */
export function isWebhookConfigError(error) {
 ...
 */
export function getServiceNameFromTool(toolName) {
    if (!toolName)
        return 'service';
    if (toolName.includes('email') || toolName.includes('thread')) {
        return 'email';
