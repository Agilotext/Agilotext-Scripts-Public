...
import { validateEmailGenerationInputs, sanitizeForLlm, validateJobId, } from "./security/input-validator.js";
...
const handleTool = async (fn, toolName) => {
    ...
    try {
        ...
        return {
            content: [{ type: "text", text: `❌ ${errorDetails.message}` }],
            isError: true,
        };
    }
};
// Helper to validate jobId before API calls
const validateJobIdWrapper = (jobId, fn, toolName) => {
    try {
        validateJobId(jobId);
        ...
        return {
            content: [{ type: "text", text: `❌ Invalid jobId: ${error.message}` }],
            isError: true,
        };
    }
};
...
const buildWebhookHeaders = () => {
    ...
    if (config.MCP_WEBHOOK_TOKEN)
        headers["Authorization"] = "Bearer " + config.MCP_WEBHOOK_TOKEN;
    return headers;
};
async function callWebhook(url, payload, toolName, envName) {
    if (!url) {
        const friendlyError = translateWebhookError("Webhook non configuré pour " + toolName + ". Définissez " + envName + " dans .env", { toolName, envName, isWebhook: true });
...
function deriveEmailStyleFingerprint(raw) {
...
server.tool("list_jobs", "List transcription jobs with optional filters. Returns filtered results based on status, date range, and filename.", {
    ...
    try {
        ...
        if (status && status !== "ALL") {
            jobs = jobs.filter((j) => j.transcriptStatus === status);
        }
        if (dateFrom) {
            const from = new Date(dateFrom);
            jobs = jobs.filter((j) => new Date(j.creationDate) >= from);
        }
        if (dateTo) {
            const to = new Date(dateTo);
            jobs = jobs.filter((j) => new Date(j.creationDate) <= to);
        }
        if (filenameContains) {
            const search = filenameContains.toLowerCase();
            jobs = jobs.filter((j) => j.filename?.toLowerCase().includes(search));
        }
        return {
            content: [{
                    type: "text",
...
server.tool("delete_job", "Delete a transcription job", { jobId: z.string() }, async ({ jobId }) => {
...
server.tool("batch_delete_jobs", "Delete multiple jobs at once. Returns success/failure count.", { jobIds: z.array(z.string()).describe("Array of job IDs to delete") }, async ({ jobIds }) => {
    try {
        const results = await Promise.allSettled(jobIds.map(id => client.deleteJob(id)));
...
// ============ TRANSCRIPTION ============
server.tool("upload_audio", "Upload and transcribe an audio file. Supports: local path, URL, or base64 encoded content.", {
    ...
    try {
        ...
        const { fileBuffer, filename: finalFilename } = await handleFileUpload(filePath, fileUrl, fileBase64, filename);
...
server.tool("transcribe_youtube", "Transcribe a YouTube video", {
    url: z.string().url(),
    timestampTranscript: z.boolean().default(true),
    doSummary: z.boolean().default(true),
    speakersExpected: z.number().default(0),
}, async ({ url, timestampTranscript, doSummary, speakersExpected }) => handleTool(() => client.sendYoutubeUrl(url, { timestampTranscript, doSummary, speakersExpected })));
// ============ PROMPTS ============
server.tool("list_prompts", "List all user prompt models", {}, async () => {
...
server.tool("list_standard_prompts", "List standard/default prompt models provided by Agilotext", {}, async () => {
...
server.tool("get_prompt_content", "Get the full text content of a prompt model", { promptId: z.string() }, async ({ promptId }) => handleTool(() => client.getPromptModelContent(promptId)));
server.tool("get_prompt_status", "Get status of a prompt model (active, pending, etc.)", { promptId: z.string() }, async ({ promptId }) => handleTool(() => client.getPromptModelUserStatus(promptId)));
server.tool("create_prompt", "Create a new prompt model with name and content", { name: z.string(), content: z.string() }, async ({ name, content }) => {
...
server.tool("update_prompt", "Update an existing prompt model content", { promptId: z.string(), content: z.string() }, async ({ promptId, content }) => {
    const result = await handleTool(() => client.updatePromptModelUser(promptId, content), "update_prompt");
    promptsCache.delete("user_prompts");
    return result;
});
server.tool("rename_prompt", "Rename a prompt model", { promptId: z.string(), newName: z.string() }, async ({ promptId, newName }) => {
    const result = await handleTool(() => client.renamePromptModel(promptId, newName), "rename_prompt");
    promptsCache.delete("user_prompts");
    return result;
});
server.tool("set_default_prompt", "Set a prompt model as the default for new transcriptions", { promptId: z.string() }, async ({ promptId }) => {
    const result = await handleTool(() => client.setPromptModelUserDefault(promptId), "set_default_prompt");
    promptsCache.delete("user_prompts");
    return result;
});
server.tool("delete_prompt", "Delete a prompt model", { promptId: z.string() }, async ({ promptId }) => {
    const result = await handleTool(() => client.deletePromptModel(promptId), "delete_prompt");
    promptsCache.delete("user_prompts");
    return result;
});
server.tool("get_prompt_template", "Get the template file for a prompt model", { promptId: z.string() }, async ({ promptId }) => handleTool(() => client.receivePromptModelTemplate(promptId)));
// ============ REPROMPT ============
server.tool("reprompt_transcript", "Apply a different prompt to an existing transcript to generate a new summary", { jobId: z.string(), promptId: z.string() }, async ({ jobId, promptId }) => handleTool(() => client.rePromptTranscript(jobId, promptId)));
server.tool("get_reprompt_status", "Get status of a reprompt operation", { jobId: z.string(), promptId: z.string() }, async ({ jobId, promptId }) => validateJobIdWrapper(jobId, () => client.getRePromptStatus(jobId, promptId), "get_reprompt_status"));
server.tool("download_reprompt", "Download the result of a reprompt operation", { jobId: z.string(), promptId: z.string(), format: z.enum(["txt", "html"]).default("html") }, async ({ jobId, promptId, format }) => validateJobIdWrapper(jobId, () => client.receiveRepromptText(jobId, promptId, format), "download_reprompt"));
// ============ WORD BOOST / LEXICON ============
server.tool("list_wordboosts", "List all word boost dictionaries", {}, async () => {
...
server.tool("get_wordboost", "Get words in a specific word boost dictionary", { wordBoostId: z.string() }, async ({ wordBoostId }) => handleTool(() => client.getWordBoost2(wordBoostId)));
server.tool("create_wordboost", "Create or update a word boost dictionary with custom words", { name: z.string(), words: z.array(z.string()) }, async ({ name, words }) => {
    const result = await handleTool(() => client.setWordBoost2(name, words), "create_wordboost");
    wordboostsCache.delete("wordboosts");
    return result;
});
server.tool("rename_wordboost", "Rename a word boost dictionary", { wordBoostId: z.string(), newName: z.string() }, async ({ wordBoostId, newName }) => {
    const result = await handleTool(() => client.renameWordBoost2(wordBoostId, newName), "rename_wordboost");
    wordboostsCache.delete("wordboosts");
    return result;
});
server.tool("set_default_wordboost", "Set a word boost dictionary as default for new transcriptions", { wordBoostId: z.string() }, async ({ wordBoostId }) => {
    const result = await handleTool(() => client.setWordBoostDefault2(wordBoostId), "set_default_wordboost");
    wordboostsCache.delete("wordboosts");
    return result;
});
server.tool("delete_wordboost", "Delete a word boost dictionary", { wordBoostId: z.string() }, async ({ wordBoostId }) => {
    const result = await handleTool(() => client.deleteWordBoost2(wordBoostId), "delete_wordboost");
    wordboostsCache.delete("wordboosts");
    return result;
});
// ============ USER PREFERENCES ============
server.tool("get_user_preferences", "Get user model preferences (AI model, etc.)", {}, async () => handleTool(() => client.getUserModelPreference()));
server.tool("set_user_model_preference", "Set preferred AI model for transcription/summary", { modelPreference: z.string() }, async ({ modelPreference }) => handleTool(() => client.setUserModelPreference(modelPreference)));
server.tool("get_send_defaults", "Get default settings for audio uploads", {}, async () => handleTool(() => client.getUserSendDefaults()));
server.tool("get_mail_notifications", "Get email notification preferences", {}, async () => handleTool(() => client.getMailNotifyType()));
server.tool("set_mail_notifications", "Set email notification preferences", { notifyType: z.string() }, async ({ notifyType }) => handleTool(() => client.setMailNotifyType(notifyType)));
// ============ WEBHOOKS ============
server.tool("create_webhook", "Create a webhook for job completion notifications", { webhookUrl: z.string().url(), eventType: z.string() }, async ({ webhookUrl, eventType }) => handleTool(() => client.webhookCreate(webhookUrl, eventType)));
server.tool("get_webhook_status", "Get current webhook configuration status", {}, async () => handleTool(() => client.webhookGetStatus()));
server.tool("resend_webhook", "Resend webhook notification for a specific job", { jobId: z.string() }, async ({ jobId }) => handleTool(() => client.webhookResend(jobId)));
// ============ CONCATENATION ============
server.tool("concatenate_jobs", "Merge multiple transcription jobs into one", { jobIds: z.array(z.string()), newName: z.string() }, async ({ jobIds, newName }) => handleTool(() => client.sendForConcatenation(jobIds, newName)));
server.tool("get_concat_status", "Get status of a concatenation job", { concatJobId: z.string() }, async ({ concatJobId }) => handleTool(() => client.getConcatStatus(concatJobId)));
server.tool("list_concat_jobs", "List concatenation jobs", { limit: z.number().default(20), offset: z.number().default(0) }, async ({ limit, offset }) => handleTool(() => client.getConcatJobsInfo(limit, offset)));
// ============ UTILITIES ============
server.tool("anonymize_text", "Anonymize personal data in text (names, addresses, etc.)", { text: z.string() }, async ({ text }) => handleTool(() => client.anonText(text)));
server.tool("get_usage_stats", "Get number of uploads for a period (day, week, month)", { period: z.enum(["day", "week", "month"]).default("month") }, async ({ period }) => handleTool(() => client.getNumberOfUploadsForPeriod(period)));
server.tool("get_api_version", "Get Agilotext API version", {}, async () => handleTool(() => client.getVersion()));
server.tool("cleanup_old_jobs", "Delete jobs older than specified days", { daysOld: z.number().default(30) }, async ({ daysOld }) => handleTool(() => client.cleanupOldJobs(daysOld)));
server.tool("get_automation_token", "Generate a new automation token for external integrations", {}, async () => handleTool(() => client.getNewAutomationToken()));
server.tool("set_google_drive_url", "Configure Google Drive integration URL", { googleDriveUrl: z.string().url() }, async ({ googleDriveUrl }) => handleTool(() => client.setGoogleDriveUrl(googleDriveUrl), "set_google_drive_url"));
// ============ UNEXPOSED API METHODS ============
server.tool("get_wordboost_status", "Get the processing status of a wordboost", { wordBoostId: z.string() }, async ({ wordBoostId }) => handleTool(() => client.getStatusWordBoost2(wordBoostId), "get_wordboost_status"));
server.tool("set_send_defaults", "Set default options for new transcriptions", {
...
// ============ WRAPPER UTILITIES ============
server.tool("wait_for_job", "Wait until a job reaches a specific status (polls every 10s). Returns when READY, READY_SUMMARY_READY, or ON_ERROR.", {
    ...
    try {
        ...
        while ((Date.now() - startTime) < maxWaitSeconds * 1000) {
            ...
            if (targetStatus === "ANY_READY" && (currentStatus === "READY" || currentStatus === "READY_SUMMARY_READY")) {
                return { content: [{ type: "text", text: JSON.stringify({ ready: true, status: currentStatus, jobId }, null, 2) }] };
            }
            if (currentStatus === targetStatus) {
                return { content: [{ type: "text", text: JSON.stringify({ ready: true, status: currentStatus, jobId }, null, 2) }] };
            }
            if (currentStatus === "ON_ERROR") {
                return { content: [{ type: "text", text: JSON.stringify({ ready: false, status: "ON_ERROR", error: status.errorMessage, jobId }, null, 2) }], isError: true };
            }
            // Wait before next poll
            await new Promise(resolve => setTimeout(resolve, pollInterval));
        }
        return {
            content: [{ type: "text", text: `❌ Timeout: Job ${jobId} did not reach ${targetStatus} within ${maxWaitSeconds}s` }],
            isError: true
        };
    }
    catch (error) {
        ...
        return { content: [{ type: "text", text: `❌ ${errorDetails.message}` }], isError: true };
    }
});
server.tool("upload_and_wait", "Upload audio, wait for completion, and return the transcript + summary. All-in-one workflow.", {
    ...
    try {
        ...
        while ((Date.now() - startTime) < maxWaitSeconds * 1000) {
            ...
            if (currentStatus === "ON_ERROR") {
                return { content: [{ type: "text", text: `❌ Job failed: ${status.errorMessage}` }], isError: true };
            }
            await new Promise(resolve => setTimeout(resolve, pollInterval));
        }
        return { content: [{ type: "text", text: `❌ Timeout waiting for job ${jobId}` }], isError: true };
    }
    catch (error) {
        const errorDetails = formatError(error);
...
server.tool("batch_download", "Download transcripts for multiple jobs in parallel", {
    ...
    try {
        ...
        for (const jobId of jobIds) {
            try {
                ...
                return {
                    content: [{ type: "text", text: `❌ Invalid jobId: ${jobId} - ${error.message}` }],
                    isError: true,
                };
            }
        }
        ...
        for (let i = 0; i < jobIds.length; i += maxConcurrent) {
            ...
            const batchResults = await Promise.allSettled(batch.map(async (jobId) => {
        ...
        const failures = results.map((r, i) => r.status === 'rejected' ? { jobId: jobIds[i], error: r.reason?.message } : null).filter(Boolean);
        return {
            content: [{
                    type: "text",
                    text: JSON.stringify({
...
// ============ SEMANTIC SEARCH ============
server.tool("search_transcripts", "🔍 Search across all transcripts for specific content, keywords, or topics. Returns matching jobs with context.", {
    ...
    try {
        ...
        if (dateFrom) {
            const from = new Date(dateFrom);
            jobsList = jobsList.filter((j) => new Date(j.creationDate) >= from);
        }
        if (dateTo) {
            const to = new Date(dateTo);
            jobsList = jobsList.filter((j) => new Date(j.creationDate) <= to);
        }
        // Only search ready jobs
        jobsList = jobsList.filter((j) => j.transcriptStatus === "READY" || j.transcriptStatus === "READY_SUMMARY_READY");
        // Search in transcripts (parallel)
        const searchResults = await Promise.allSettled(jobsList.slice(0, 30).map(async (job) => {
            try {
...
server.tool("extract_meeting_insights", "🧠 Extract action items, decisions, questions, and key topics from a transcript. AI-powered meeting intelligence.", {
...
server.tool("fetch_email_thread", "Find emails in your inbox", {
    ...
    try {
        ...
        const data = await callWebhook(config.MCP_EMAIL_THREAD_WEBHOOK, {
        ...
        }, "fetch_email_thread", "MCP_EMAIL_THREAD_WEBHOOK");
...
server.tool("fetch_calendar_context", "Find meetings in your calendar", {
    ...
    try {
        ...
        const data = await callWebhook(config.MCP_CALENDAR_WEBHOOK, { action: "fetch_calendar_context", source: detectedSource, eventId, title, date, participants, timezone }, "fetch_calendar_context", "MCP_CALENDAR_WEBHOOK");
        return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    }
    catch (error) {
...
server.tool("find_my_emails", "Search and find emails in your inbox (read-only)", {
    ...
    try {
        ...
        const data = await callWebhook(config.MCP_EMAIL_THREAD_WEBHOOK, {
            ...
            query,
        }, "find_my_emails", "MCP_EMAIL_THREAD_WEBHOOK");
        return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    }
    catch (error) {
...
server.tool("find_my_meetings", "Find meetings in your calendar (read-only)", {
    ...
    try {
        ...
        const data = await callWebhook(config.MCP_CALENDAR_WEBHOOK, {
            ...
            participants,
            timezone
        }, "find_my_meetings", "MCP_CALENDAR_WEBHOOK");
        return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    }
    catch (error) {
...
server.tool("crm_upsert_contact", "Add or update a contact in your CRM", {
    ...
    try {
        const data = await callWebhook(config.MCP_CRM_WEBHOOK, { action: "upsert_contact", email, name, company, phone, externalId, tags, metadata }, "crm_upsert_contact", "MCP_CRM_WEBHOOK");
        return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    }
    catch (error) {
        const errorDetails = formatError(error);
...
server.tool("crm_attach_transcript", "Link a transcript to a contact in your CRM", {
    ...
    try {
        ...
        const data = await callWebhook(config.MCP_CRM_WEBHOOK, { action: "attach_transcript", jobId, contactId, email, title, summary, summaryFormat, sharedUrl }, "crm_attach_transcript", "MCP_CRM_WEBHOOK");
        return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    }
    catch (error) {
...
server.tool("extract_email_style", "Extract a style fingerprint from an email thread", {
    ...
    try {
        ...
        return { content: [{ type: "text", text: JSON.stringify(fingerprint, null, 2) }] };
    }
    catch (error) {
        const errorDetails = formatError(error);
...
server.tool("build_context_pack", "Build a unified context pack for follow-up tasks (email/CRM)", {
    jobId: z.string(),
    includeSummary: z.boolean().default(true),
    ...
    try {
        validateJobId(jobId);
        const pack = { jobId, emailThread, crmContext, calendarContext };
        ...
        if (includeTranscript || includeInsights) {
            ...
            if (includeTranscript)
                pack.transcript = transcriptText;
        }
        if (includeSummary) {
...
server.tool("sync_contact_and_attach", "Upsert a CRM contact then attach transcript/summary in one call", {
    jobId: z.string(),
    email: z.string().email(),
    name: z.string().optional(),
    company: z.string().optional(),
    ...
    try {
        ...
        if (!resolvedId) {
            upsertResp = await callWebhook(config.MCP_CRM_WEBHOOK, { action: "upsert_contact", email, name, company, phone, tags, metadata }, "sync_contact_and_attach", "MCP_CRM_WEBHOOK");
        ...
        const attachResp = await callWebhook(config.MCP_CRM_WEBHOOK, { action: "attach_transcript", jobId, contactId: resolvedId, email, title, summary, summaryFormat, sharedUrl }, "sync_contact_and_attach", "MCP_CRM_WEBHOOK");
        return { content: [{ type: "text", text: JSON.stringify({ upsert: upsertResp, attach: attachResp }, null, 2) }] };
    }
    catch (error) {
        const errorDetails = formatError(error);
...
server.tool("generate_followup_email", "Create a follow-up email draft (draft only)", {
    jobId: z.string(),
    ...
    try {
        ...
        const data = await callWebhook(config.MCP_EMAIL_DRAFT_WEBHOOK, {
        ...
        }, "generate_followup_email", "MCP_EMAIL_DRAFT_WEBHOOK");
        ...
        return { content: [{ type: "text", text: JSON.stringify(validatedData, null, 2) }] };
    }
    catch (error) {
        const errorDetails = formatError(error);
...
server.tool("suggest_prompt", "💡 Analyze a transcript and suggest the best prompt model to use. AI-powered recommendation.", {
    jobId: z.string().describe("The job ID to analyze"),
}, async ({ jobId }) => {
    try {
        validateJobId(jobId);
            ...
            content: [{
                    ...
                    text: JSON.stringify({
                        ...
                        suggestedPrompts: suggestedPrompts.length > 0
                            ? suggestedPrompts.map((p) => ({ id: p.promptId || p.id, name: p.promptName || p.name }))
...
// ============ CROSS-JOB INSIGHTS ============
server.tool("get_job_analytics", "📊 Get advanced analytics across all your transcription jobs: trends, patterns, usage stats.", {
    ...
    try {
        ...
        const periodJobs = jobsList.filter((j) => new Date(j.creationDate) >= periodStart);
        // Status distribution
        const statusCounts = {};
        ...
        // Success rate
        const completed = periodJobs.filter((j) => j.transcriptStatus === "READY" || j.transcriptStatus === "READY_SUMMARY_READY").length;
        const failed = periodJobs.filter((j) => j.transcriptStatus === "ON_ERROR").length;
        const pending = periodJobs.filter((j) => j.transcriptStatus === "PENDING").length;
        return {
            content: [{
                    type: "text",
                    text: JSON.stringify({
...
// ============ TIME-BASED QUERIES ============
server.tool("query_by_time", "🕒 Natural language time queries: 'What was discussed last week?', 'Jobs from yesterday', etc.", {
    timeQuery: z.enum([
    ...
    try {
        ...
        const jobs = await client.getJobsInfo(100, 0);
        const filtered = (jobs.jobsInfoDtos || []).filter((j) => {
            const jobDate = new Date(j.creationDate);
            return jobDate >= startDate && jobDate <= endDate;
        });
        let results = filtered.map((j) => ({
            jobId: j.jobId,
            filename: j.filename,
            date: j.creationDate,
            status: j.transcriptStatus,
        ...
        if (includeContent && results.length <= 5) {
            results = await Promise.all(results.map(async (job) => {
                try {
...
// ============ SPEAKER ANALYSIS ============
server.tool("analyze_speakers", "👥 Analyze speakers in a transcript: talk time, patterns, speaking order.", {
    jobId: z.string().describe("The job ID to analyze"),
}, async ({ jobId }) => {
    try {
        validateJobId(jobId);
...
// ============ COMPARE TRANSCRIPTS ============
server.tool("compare_transcripts", "🔄 Compare two transcripts or two prompt outputs. Find differences and similarities.", {
...
server.tool("bulk_reprompt", "⚡ Apply a prompt to multiple jobs at once. Great for reprocessing with a new/updated prompt.", {
    jobIds: z.array(z.string()).describe("Array of job IDs"),
        ...
        for (let i = 0; i < jobIds.length; i += actualMaxConcurrent) {
            ...
            batchResults.forEach((r, idx) => {
                if (r.status === 'fulfilled') {
                    ...
                    results.push({ jobId: batch[idx], status: "failed", error: r.reason?.message });
                }
            });
...
// ============ EXPORT & INTEGRATION ============
server.tool("export_to_json", "📦 Export multiple jobs to a structured JSON file with all metadata, transcripts, and summaries.", {
    ...
    try {
        ...
        const exports = await Promise.allSettled(jobIds.map(async (jobId) => {
            const status = await client.getTranscriptStatus(jobId);
            const data = {
                jobId,
