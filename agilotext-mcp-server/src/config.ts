...
const envSchema = z
    ...
    .refine(
        (d) => {
            ...
            const pwd =
                (d.AGILOTEXT_PASSWORD && d.AGILOTEXT_PASSWORD.length > 0 && d.AGILOTEXT_PASSWORD) ||
        ...
        {
            message:
                "Définir AGILOTEXT_TOKEN, ou l’un de AGILOTEXT_PASSWORD / AGILOTEXT_APP_PASSWORD / AGILOTEXT_ADMIN_PASSWORD (post https://api.agilotext.com/api/v1/getAuthToken)",
        }
    );
...
function pickAccountPassword(): string | undefined {
...
export const config = {
    ...
    accountPassword: pickAccountPassword() as string | undefined,
} as const;
