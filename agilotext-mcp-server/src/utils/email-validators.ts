 ...
 */
export function validateEmailDraftResponse(data: any): EmailDraftResponse {
  try {
    return EmailDraftResponseSchema.parse(data);
  } catch (error) {
    if (error instanceof z.ZodError) {
      throw new Error(
        `Invalid email draft response: ${error.errors.map((e) => e.message).join(", ")}`
      );
    }
    throw error;
  }
 ...
 */
export function validateEmailThreadResponse(data: any): EmailThreadResponse {
  try {
    return EmailThreadResponseSchema.parse(data);
  } catch (error) {
    if (error instanceof z.ZodError) {
      throw new Error(
        `Invalid email thread response: ${error.errors.map((e) => e.message).join(", ")}`
      );
    }
    throw error;
  }
...
export function validateSenderIdentity(
  ...
  if (fromEmail.toLowerCase() !== authenticatedEmail.toLowerCase()) {
    throw new Error(
      `From email ${fromEmail} does not match authenticated account ${authenticatedEmail}`
    );
  }
}
 ...
 */
export function preserveReplySubject(originalSubject: string): string {
  if (!originalSubject) {
    return "";
  }
