...
import {
  ...
  sanitizeFilename,
  validateFileSize,
} from "../security/file-validator.js";
...
export async function handleFileUpload(
  ...
  // Option 3: Base64 encoded
    ...
    if (estimatedSize > MAX_BASE64_SIZE) {
      throw new Error(
        `Base64 data too large: estimated ${(estimatedSize / (1024 * 1024)).toFixed(2)}MB exceeds maximum of ${(MAX_BASE64_SIZE / (1024 * 1024)).toFixed(2)}MB`
      );
    }
  ...
  // No input provided
    ...
    throw new Error(
      "You must provide one of: filePath, fileUrl, or fileBase64"
    );
  }
