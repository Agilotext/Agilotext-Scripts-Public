/* @agilotext/agilo-prompt-atelier v1.10 — fork studio 1.09 + Enregistrer sous */
(function () {
  "use strict";
...
class AgilotextPromptsClient {
    ...
    async duplicatePromptModel(sourcePromptId, promptName) {
...
class StudioApp {
    ...
    renderDetail(mainCol, errBox) {
        ...
        if (!this.cfg.readOnly) {
            ...
            const saveAs = el("button", "agilo-ps-btn agilo-ps-btn--secondary button is-secondary", "Enregistrer sous");
            saveAs.type = "button";
            saveAs.title = "Crée une copie (nouveau id), sans écraser ce modèle.";
            saveAs.addEventListener("click", () => void this.saveAsCopy(errBox));
            saveGroup.append(saveAs);
            this.registerSaveBtn(saveAs);
    ...
    async saveAsCopy(errBox) {
        const s = this.state;
        if (!s || this.saving)
            return;
        const suggested = `${s.promptName} (copie)`.slice(0, 120);
        const raw = window.prompt("Nom du nouveau modèle", suggested);
        if (raw == null)
            return;
        const promptName = raw.trim();
        if (!promptName) {
            errBox.hidden = false;
        ...
        this.setSaving(true);
        errBox.hidden = true;
        this.setStatus("Enregistrer sous… copie en cours.", true);
        try {
            const res = await this.client.duplicatePromptModel(s.promptId, promptName);
            const newId = res && (res.promptId ?? res.promptModelId);
            if (newId == null || String(newId).trim() === "") {
                throw new Error("La copie n’a pas renvoyé d’identifiant. Réessayez ou contactez le support.");
            }
            const copyId = String(newId);
            const ok = await this.client.waitPromptReady(copyId, {
                onTick: ({ elapsedMs, maxMs }) => {
                    const sec = Math.round(elapsedMs / 1000);
                    this.setStatus(`Finalisation de la copie… (${sec}s / ~${Math.round(maxMs / 60000)} min max).`, true);
                },
            });
            if (!ok) {
                throw new Error("La copie n’est pas repassée à READY. Réessayez plus tard.");
            }
            try {
                window.dispatchEvent(new CustomEvent("agilo-ps-models-changed", { detail: { promptId: copyId, promptName } }));
            }
            catch (_e) {
                /* ignore */
            }
            this.setStatus("Copie créée. Ouverture du nouveau modèle…", false);
            if (this.mainColRef) {
                await this.loadPrompt(copyId, promptName, this.mainColRef, errBox);
            }
            this.setStatus(`Modèle ${copyId} ouvert.`, false);
