...
// 
// ========== VERSION V01.01 (26/01/2026) ==========
// CORRECTION: Amélioration de la détection des styles globaux
// - Ajout de la détection de <style> pour utiliser l'iframe systématiquement
// - Résout le bug de la barre d'onglets qui disparaît sur Chrome/Chromium
// ================================================
...
function mapNicoJsonToSegments(j){
  ...
  function injectSummaryContent(html) {
    ...
    // ✅ V01.01 : Détection AMÉLIORÉE des styles globaux
    // On utilise l'iframe dès qu'il y a une balise <style> dans le HTML
    // Cela couvre TOUS les cas de figure et évite les conflits CSS
    const hasGlobalStyles = html.includes('<style') ||  // ✅ NOUVEAU: Tout HTML avec <style>
                            html.includes('<head') || 
                            html.includes('<body') || 
                            /\*\s*\{/.test(html) ||
                            /body\s*\{/.test(html);
