...
(function () {
  ...
  window.__agiloTranscriptHistoryVersion = '1.0.0';
  ...
  function isTranscriptVisible() {
    ...
    if (tab) return tab.id === 'tab-transcript';
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .agilo-tx-hist__undo{gap:6px;color:#174a96;font-family:inherit;font-size:13px;font-weight:600;line-height:1.2;padding:6px 4px}
  ...
  function ensureRoot() {
    ...
    root.innerHTML = `<button type="button" class="agilo-tx-hist__undo">${UNDO}<span>Revenir</span></button>` +
      `<button type="button" class="agilo-tx-hist__clock" aria-label="Historique des transcriptions" aria-expanded="false" aria-haspopup="dialog">${CLOCK}</button>` +
  ...
  window.__agiloTxHistoryHelpers = { parseApiDate, cleanSavedList, validateTranscript, canonicalSegments, sameTranscript, restoreTransaction };
