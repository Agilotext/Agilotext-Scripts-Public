...
<body>
  ...
  function loadHistory(){
    ...
    if (!MESSAGES.length){
      MESSAGES.push({ role:'system', text:'Bienvenue dans l'assistant IA. Posez vos questions sur le transcript.', t:new Date().toISOString() });
    }
  }
...
async function rePromptSubmit(args){
  ...
  try{
    const prompt = String(hiddenPrompt||'').trim();
