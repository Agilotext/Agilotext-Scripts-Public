...
(function ready(fn) {
  ...
  let _bulkBar = null, _bulkBarCount = null, _bulkDelBtn = null, _bulkRenameBtn = null, _bulkReason = null;
  let _activeSelectionPickerClose = null;
  ...
  // Compare the saved labels as-is. Trimming is used only to reject an empty label.
  function selectionStatusForSegments(indices, segments) {
    const selected = Array.from(indices).sort((a, b) => a - b);
    if (!Array.isArray(segments) || selected.some(i => !Number.isInteger(i) || i < 0 || i >= segments.length)) {
      return { eligible: false, selected, reason: 'La sélection a changé. Recommencez.' };
    }
    const labels = selected.map(i => String(segments[i]?.speaker ?? ''));
    if (labels.some(label => !label.trim())) {
      return { eligible: false, selected, displayLabel: 'locuteur non renseigné', reason: 'Un passage sélectionné est sans locuteur.' };
    }
    if (labels.some(label => label !== labels[0])) {
      return { eligible: false, selected, displayLabel: 'locuteurs différents', reason: 'Locuteurs différents (casse et espaces compris).' };
    }
    const displayLabel = labels.length ? `« ${labels[0]} »` : '';
    if (selected.length < 2) return { eligible: false, selected, displayLabel, reason: 'Sélectionnez au moins 2 passages.' };
    return { eligible: true, selected, oldName: labels[0], displayLabel, reason: '' };
  }
  ...
  function captureSelectionSnapshot() {
    const status = currentSelectionStatus();
  ...
  function isSelectionSnapshotCurrent(snapshot) {
    if (!snapshot || snapshot.revision !== _selectionRevision) return false;
    const current = currentSelectionStatus();
    const root = editors.transcript;
    return current.eligible && current.oldName === snapshot.oldName
  ...
  function clearSegSelection() {
    if (_activeSelectionPickerClose) _activeSelectionPickerClose(false);
    const root = editors.transcript;
  ...
  function updateBulkBar() {
    ...
    const status = currentSelectionStatus();
    if (_bulkBarCount) {
      const n = _selectedSegs.size;
      _bulkBarCount.textContent = `${n} passage${n > 1 ? 's' : ''} sélectionné${n > 1 ? 's' : ''}`
        + (status.displayLabel ? ` · ${status.displayLabel}` : '');
    }
    if (_bulkRenameBtn) _bulkRenameBtn.disabled = !status.eligible;
    if (_bulkReason) {
      _bulkReason.textContent = status.reason;
      _bulkReason.hidden = !status.reason;
    }
  ...
  function ensureBulkBar() {
    ...
    _bulkBar.setAttribute('hidden', '');
    _bulkBar.setAttribute('role', 'toolbar');
    _bulkBar.setAttribute('aria-label', 'Actions sur les passages sélectionnés');
    _bulkBarCount = document.createElement('span');
    _bulkBarCount.className = 'agilo-bulk-bar__count';
    _bulkBarCount.setAttribute('aria-live', 'polite');
    _bulkReason = document.createElement('span');
    _bulkReason.id = 'agilo-bulk-reason';
    _bulkReason.className = 'agilo-bulk-bar__reason';
    _bulkReason.hidden = true;
    _bulkRenameBtn = document.createElement('button');
    _bulkRenameBtn.type = 'button';
    _bulkRenameBtn.className = 'agilo-bulk-bar__btn agilo-bulk-bar__btn--rename';
    _bulkRenameBtn.textContent = 'Renommer la sélection';
    _bulkRenameBtn.setAttribute('aria-describedby', 'agilo-bulk-reason');
    _bulkRenameBtn.addEventListener('click', () => { openSelectedRename(); });
    ...
    clear.type = 'button';
    clear.className = 'agilo-bulk-bar__btn agilo-bulk-bar__btn--clear';
    clear.textContent = 'Effacer la sélection';
    clear.addEventListener('click', clearSegSelection);
    _bulkBar.appendChild(_bulkBarCount);
    _bulkBar.appendChild(_bulkRenameBtn);
    _bulkBar.appendChild(_bulkDelBtn);
    _bulkBar.appendChild(clear);
    _bulkBar.appendChild(_bulkReason);
    ...
    if (!root.__bound) {
      ...
      if (!root.__agiloDocKeys) {
        ...
        document.addEventListener('click', (e) => {
          ...
          if (editors.transcript?.contains(e.target)) return;
          if (_bulkBar?.contains(e.target)) return;
          if (e.target.closest?.('.ag-speaker-picker, .ag-rename-menu')) return;
          clearSegSelection();
        }, true);
        document.addEventListener('keydown', (e) => {
  ...
  function ag_applyRenameScope({ scope, oldName, newName, idx, indices = [] }) {
    const root = editors.transcript; if (!root) return 0;
    const targets = [];
    if (scope === 'one') targets.push(idx);
    else if (scope === 'selected') targets.push(...indices);
  ...
  function ag_bindAnchoredPopover(panel, anchor, { onClose, ignoreOutside } = {}) {
    const off = [];
    let raf = 0;
    let unbound = false;
    ...
    on(document, 'click', (e) => {
      ...
      if (typeof ignoreOutside === 'function' && ignoreOutside(e.target)) return;
      unbind();
      if (typeof onClose === 'function') onClose();
    }, true);
  ...
  function ag_showSpeakerPicker(anchor, { currentName, names, onPick, onCancel, selectionMode = null } = {}) {
    if (_activeSelectionPickerClose) _activeSelectionPickerClose(false);
    ...
    const selectedCount = selectionMode?.count || 0;
    ...
    let nameInput = null;
    const spell = (selectionMode || person) ? document.createElement('div') : null;
    if (spell) {
      spell.className = 'ag-speaker-picker__spell';
      const title = document.createElement('div');
      title.className = 'ag-speaker-picker__spell-title';
      title.textContent = selectionMode
        ? `Renommer ${selectedCount} passages de « ${currentName} »`
        : 'Corriger « ' + currentName + ' »';
      ...
      go.textContent = selectionMode ? `Renommer ${selectedCount} passages` : 'Valider';
      function validateSpell(ev) {
        if (ev) { ev.preventDefault(); ev.stopPropagation(); }
        const joined = String(nameInput.value || '').trim();
        if (!joined || joined === currentName) { close(true); return; }
        const scope = selectionMode ? 'selected' : ((applyAllBox && applyAllBox.checked && spellCount > 1) ? 'all' : 'one');
        close(false);
      ...
      if (!selectionMode) spell.appendChild(row);
    ...
    function close(cancel) {
      if (closed) return;
      closed = true;
      if (_activeSelectionPickerClose === close) _activeSelectionPickerClose = null;
      bound?.unbind();
      ag_closeSpeakerPicker();
    ...
    function pick(raw) {
      ...
      close(false);
      if (typeof onPick === 'function') onPick(value, selectionMode ? 'selected' : undefined);
    }
    function tryCreateFromQuery() {
      if (!shouldCreateSpeakerFromQuery(query, allNames)) return false;
      const n = normalizeName(query);
      if (!n) return false;
      if (!selectionMode) pushStoredRoster(getJobIdForRoster(), n);
      pick(n);
      return true;
    }
    ...
    function spellScope() {
      if (selectionMode) return 'selected';
    ...
    renderList();
    if (selectionMode) _activeSelectionPickerClose = close;
    bound = ag_bindAnchoredPopover(panel, anchor, {
      onClose() { close(true); },
      ignoreOutside: selectionMode ? (target) => !!_bulkBar?.contains(target) : null
    });
    try {
      if (nameInput) {
  ...
  function openSelectedRename() {
    const snapshot = captureSelectionSnapshot();
    if (!snapshot) {
      updateBulkBar();
      toast('Sélection invalide. Sélectionnez au moins 2 passages du même locuteur.');
      return;
    }

    function applySelectedName(rawName) {
      if (!isSelectionSnapshotCurrent(snapshot)) {
        clearSegSelection();
        toast('La sélection a changé. Recommencez.');
        return;
      }
      const newName = normalizeName(rawName);
      if (!newName || newName === snapshot.oldName) {
        clearSegSelection();
        return;
      }
      try { syncDomToModel(); } catch { }
      const count = ag_applyRenameScope({
        scope: 'selected', oldName: snapshot.oldName, newName, indices: snapshot.selected
      });
      if (count !== snapshot.selected.length) {
        toast('Le renommage n’a pas pu être appliqué. Recommencez.');
        clearSegSelection();
        return;
      }
      pushStoredRoster(getJobIdForRoster(), newName);
      forgetRosterNameIfUnused(snapshot.oldName);
      clearSegSelection();
      toast(`${count} passages renommés « ${snapshot.oldName} » → « ${newName} ». Cliquez Sauvegarder.`);
    }

    if (!isSpeakerPickerEnabled()) {
      const rawName = prompt('Renommer les passages sélectionnés :', snapshot.oldName);
      if (rawName == null) { clearSegSelection(); return; }
      applySelectedName(rawName);
      return;
    }
    try {
      ag_showSpeakerPicker(_bulkRenameBtn, {
        currentName: snapshot.oldName,
        names: collectRosterNames(),
        selectionMode: { count: snapshot.selected.length },
        onPick: applySelectedName,
        onCancel: clearSegSelection
      });
    } catch {
      const rawName = prompt('Renommer les passages sélectionnés :', snapshot.oldName);
      if (rawName == null) { clearSegSelection(); return; }
  ...
  window.__agiloEditorConfidenceVersion = '1.09.14-multi-select';
});
