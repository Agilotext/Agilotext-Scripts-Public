...
| `Code-main-editor-IFRAME_V04-confidence.js` | Fork V04 + hooks confidence (`__agiloEditorConfidenceVersion` = `1.09.6`) |
...
- Panneau global sticky/floating : passages à relire en premier, score en secondaire, « Passage précédent » / « Passage suivant », toggle « Passages à relire »
...
9. [ ] Panneau : « Passage précédent » et « Passage suivant » visibles quand il y a des passages
10. [ ] « Passage précédent » x5 et « Passage suivant » x5 : barre d'onglets reste visible sans reload
11. [ ] `Alt+ArrowRight` / `Alt+ArrowLeft` : même comportement, onglets stables
12. [ ] Depuis Compte rendu, « Passage suivant » bascule sur Transcription
13. [ ] Ouvrir « Télécharger transcription », « Télécharger compte rendu » et « Analyses IA » : menus entiers visibles, aucun libellé d'onglet ne traverse le menu
14. [ ] Popups upsell `.wrapper-message-pro.download`, menu renommage intervenant, bannière anonymisation : pas de recouvrement par la barre d'onglets
15. [ ] Toggle ON + scroll : onglets restent visibles (garde JS shell, pas de sticky)
16. [ ] `getComputedStyle(document.querySelector('main.ed-main nav.ed-tabs')).zIndex` retourne `'auto'`
17. [ ] Changement rapide de job : aucun badge résiduel
18. [ ] `AGILOTEXT_ENABLE_CONFIDENCE = false` : aucun appel réseau confidence (onglet Network)

**Go/no-go prod :** les 18 scénarios OK + accord Nicolas sur navigation UI.
