...
(function() {
  ...
  function agiloEditorCredsRoot() {
    ...
    if (!c || typeof c.pickEdition !== 'function') {
      throw new Error(
        '[AGILO:RELANCE] Charger agilo-editor-creds.js avant relance-compte-rendu.js (ordre des <script> dans Webflow).'
      );
    }
    return c;
  }
  ...
  // ============================================
  // RÉCUPÉRATION DES CREDENTIALS (pick* → agilo-editor-creds.js)
  // ============================================
  ...
  function updateButtonVisibility() {
    ...
    if (isSummaryTab) {
      ...
      if (noSummary) {
        const summaryPane = querySummaryEditor();
        const alertBox =
          document.querySelector('#editorRoot .ag-alert.ag-alert--warn') ||
          (summaryPane && summaryPane.querySelector('.ag-alert.ag-alert--warn'));
  ...
  /** Lit le promptId mis en cache par Code-modeles-compte-rendu.js pour ce job.
   *  sessionStorage d'abord (plus récent), localStorage en fallback. */
  function _getCachedPromptId(jobId) {
    if (!jobId) return null;
    try {
      const ss = sessionStorage.getItem('agilo:summaryPromptId:' + jobId);
      if (ss && ss !== 'null') {
        const n = Number(ss);
    ...
    try {
      const raw = localStorage.getItem('agilo:job-prompt-ids');
      if (raw) {
  ...
  async function relancerCompteRendu() {
    ...
    if (firstGen) {
      ...
      confirmMsg =
        ...
        modelNote +
        '\n\nL’interface se met à jour automatiquement dès qu’il est prêt.';
    } else {
      confirmMsg =
