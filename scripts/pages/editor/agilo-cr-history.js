...
(function () {
  ...
  const UNDO_HINT = 'Revenir à une version précédente du compte-rendu';
  const CLOCK_HINT = 'Historique des versions du compte-rendu';

  function activeTabId() {
    const tab = document.querySelector('[role="tab"][aria-selected="true"]');
    return tab ? String(tab.id || '') : '';
  }
  function isSummaryTab(tabId) { return tabId === 'tab-summary'; }
  function isSummaryVisible() { return isSummaryTab(activeTabId()); }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .agilo-cr-hist__clock:hover, .agilo-cr-hist__clock[aria-expanded="true"] { background: #eef2f7; color: #174a96; }
      .agilo-cr-hist__pop {
        display: none;
        position: absolute;
        top: calc(100% + 4px);
  ...
  function ensureRoot() {
    ...
    root.innerHTML =
      '<button type="button" class="agilo-cr-hist__undo" title="' + UNDO_HINT + '" aria-label="' + UNDO_HINT + '">' +
      ICO_UNDO +
      '<span class="agilo-cr-hist__undo-label">Revenir</span></button>' +
      '<button type="button" class="agilo-cr-hist__clock" title="' + CLOCK_HINT + '" aria-label="' + CLOCK_HINT + '" aria-expanded="false" aria-haspopup="dialog">' +
      ICO_CLOCK +
      '</button>' +
  ...
  function renderList() {
    if (!root) return;
    const show = shouldShowHistory(previous) && isSummaryVisible();
