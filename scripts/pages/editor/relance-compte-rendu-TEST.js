...
(function() {
  ...
  /**
   * Récupérer le statut du transcript via getTranscriptStatus
   */
  async function getTranscriptStatus(jobId, email, token, edition) {
    try {
      ...
      console.error('[AGILO:RELANCE] Erreur getTranscriptStatus:', error);
      return null;
    }
  }
  ...
  async function checkSummaryExists(jobId, email, token, edition) {
    try {
      ...
      console.log('[AGILO:RELANCE] Vérification existence compte-rendu:', { status, jobId });
      ...
      console.error('[AGILO:RELANCE] Erreur checkSummaryExists:', error);
      return false;
    }
  }
    ...
    document.addEventListener('click', function(e) {
      ...
      if (btn) {
        ...
        relancerCompteRendu().catch(error => {
          console.error('[AGILO:RELANCE] ❌ ERREUR:', error);
          isGenerating = false;
          hideSummaryLoading();
