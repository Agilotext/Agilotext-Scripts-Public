// Agilotext - Chat IA — variante TEST « email riche » (gras ** + puces • dans la bulle UI)
// Copie de Code-chat_V05.js : ne pas utiliser en prod ; la prod reste sur Code-chat_V05.js
// ⚠️ Chargé depuis GitHub (page Webflow de test uniquement)
window.__agiloChatVersion = 'V05-rich-email-test';

document.addEventListener('DOMContentLoaded', () => {
  if (!document.getElementById('agilo-rich-email-test-styles')) {
    const st = document.createElement('style');
    st.id = 'agilo-rich-email-test-styles';
    st.textContent = '.agilo-email-block-body strong,.agilo-email-block-subject strong{font-weight:600;}';
...
<body>
  ...
  /** Texte plat pour copie / mailto / Gmail / Outlook (pas d’astérisques). */
  ...
  function emailSubjectToDisplayHtml(s) {
    const esc = escapeHtmlBasic(s);
    return esc.replace(/\*\*([\s\S]+?)\*\*/g, '<strong>$1</strong>');
  }

  /** Paragraphe corps email : échappement + **gras** + retours ligne → <br>. */
  function emailParagraphToDisplayHtml(p) {
    const esc = escapeHtmlBasic(p);
    const withBold = esc.replace(/\*\*([\s\S]+?)\*\*/g, '<strong>$1</strong>');
    ...
    if (m.role === 'assistant') {
      ...
      if (looksLikeEmail && !isThinking && displayText.length > 10) {
        ...
        if (parsed.subject) {
          ...
          subjLine.innerHTML = '<span class="agilo-email-block-subject-label">Objet</span> ' + emailSubjectToDisplayHtml(parsed.subject);
  ...
  function postProcessEmail(text) {
    ...
    // Puces : normaliser tiret / astérisque / ▪ en « • » (affichage + cohérence modèle)
    t = t.replace(/^\s*[-*]\s+/gm, '• ');
    t = t.replace(/^\s*▪\s+/gm, '• ');
