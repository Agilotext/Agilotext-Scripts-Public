// Agilotext - Rail Changement Audio v2 (limit=50 + charger plus)
...
let __pendingLoadTimer = null;
  ...
  async function fetchJobs(auth, retried = false, overrideFilter, options = {}){
    ...
    const mapped = j.jobsInfoDtos.map(x=>{
  ...
  /* ================== Rendu ================== */
  function buildRailItem(j, current) {
  ...
  function removeRailLoadMore() {
    const btn = byId('agilo-rail-load-more');
    if (btn) btn.remove();
  }
  ...
  function appendRailItems(jobs) {
    ...
    jobs.forEach((j) => {
      rail.list.appendChild(buildRailItem(j, current));
    });
  ...
  function renderRail(jobs){
    ...
    jobs.forEach((j) => {
      const el = buildRailItem(j, current);
      if (el.classList.contains('is-active')) hasActive = true;
