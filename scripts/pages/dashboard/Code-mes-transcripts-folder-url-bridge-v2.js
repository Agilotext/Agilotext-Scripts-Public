...
(function () {
  ...
  function isServerPaginationActive() {
    return !!(window.__agiloMesTranscriptsPagination && window.__agiloMesTranscriptsPagination.enabled);
  }
  ...
  async function fetchJobsMap(auth, retried = false) {
    const url = `${API_BASE}/getJobsInfo?username=${encodeURIComponent(auth.email)}&token=${encodeURIComponent(auth.token)}&edition=${encodeURIComponent(auth.edition)}&limit=${JOBS_MAP_LIMIT}&offset=0`;
